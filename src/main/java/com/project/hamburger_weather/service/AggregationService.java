...
public class AggregationService {
    ...
    public Mono<ReportDto> getTheAnswer(AddressDto addressStart, AddressDto addressEnd) {
        ...
        Mono<RoutingServiceDto> routeMono = buildRoute(addressStart, addressEnd).cache();

        Mono<RoutingServiceDto> optimized = routeMono.map(route -> coordinatesOptimizationService.deduplicate(route.coordinates(), 3.0))
                .cache();
    ...
    public Mono<RoutingServiceDto> buildRoute(AddressDto addressStart, AddressDto addressEnd) {
    ...
    public Flux<WeatherForecastDto> getAllForecasts(Mono<RoutingServiceDto> monoRoute) {
